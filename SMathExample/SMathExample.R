source("COHb.R")
#================================================== Sample ==================================================#
XCOHb = 15*percent # COHb in blood sample (%)
Hb = 15.8*gram/(100*mL) # Hemoglobin in blood sample (grams/100mL)
#COHb.D = COHb(XCOHb=XCOHb,Hb=Hb) # Fraction of COHb in blood sample (%)
COHb.C = COHb(XCOHb=XCOHb,Hb=Hb) # Fraction of COHb in blood sample (%)
#================================================== Employee ==================================================#
#w = 166*pound # Weight (pounds)
#h = 67*inch # Height (inches)
#Vb = Vb.m(W=w,H=h) # Estimated blood volume of employee (liters)
Vb = 5*liter # Estimated blood volume of employee (liters)
#SS = 0 # Smoker Status
#XCOHb.A = XCOHb.0_s(SS=SS) # Fraction of COHb in blood prior to exposure (%)
XCOHb.A = 0.75*percent # Fraction of COHb in blood prior to exposure (%)
COHb.A = Hf*Hb*XCOHb.A # COHb in blood prior to exposure
#================================================== Enviroment ==================================================#
#z = 4000*ft # Elevation
#PB = P(z=z) # Atmospheric Pressure (mmHg)
PB = 700*mmHg # Atmospheric Pressure (mmHg)
#================================================== Oxygen Therapy ==================================================#
t_t = 0*minute # Oxygen Therapy duration (minutes)
#AL_t = 0 # Oxygen Therapy activity Level
#x.O2_t = x.O2 = 21*percent # Oxygen Therapy oxygen level (% oxygen)
...
AL_c = 0 # Clearance activity Level
x.O2_c = 21*percent # Clearance oxygen level (% oxygen)
#x.CO_c.fhs = 0*ppm # CO clearance from first hand smoke (ppm)
#x.CO_c.shs = 0*ppm # CO clearance from second hand smoke (ppm)
x.CO_c = 4*ppm # Carbon Monoxide Level (ppm)
...
AL_e = 2 # Exposure activity Level
...
#fhs_e.cigarettes = 3 # Cigarettes smoked during exposure
#x.CO_e.fhs = steadyState_c(cigs=fhs_e.cigarettes*960*minute/t_e) # CO exposure from first hand smoke (ppm)
#shs_e.time = 60*minute # Exposure to second hand smoke (minutes)
#x.CO_e.shs = steadyState_s(SS=1)*shs_e.time/t_e # CO exposure from second hand smoke (ppm)
...
pdf("SMathExamplePlot.pdf", width=10, height=5)
...
text(-3,1.7*XCOHb.A/percent,sprintf("A", x.CO_e.o/ppm),adj = c(0,0))
text(t_e/60,1.04*XCOHb.B/percent,sprintf("B", XCOHb.B/percent))
text((t_e+t_c)/60,0.9*XCOHb.C/percent,sprintf("C", XCOHb.B/percent))
dev.off()